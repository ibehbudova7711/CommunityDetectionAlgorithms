...
def run_fast_algorithms(args):
    ...
    try:
        ...
        print(f" Louvain error ({size}, {sparsity}, {graph_type}): {e}", flush=True)
    ...
    try:
        ...
        print(f" Spectral error ({size}, {sparsity}, {graph_type}): {e}", flush=True)
...
def run_gn_sequential(tasks):
    for filename, size, sparsity, graph_type in tasks:
...
def main():
    ...
    with Pool(processes=min(3, cpu_count()-1)) as pool:
